...
public partial class LevelGenerator : MonoBehaviour
{
    ...
    public void BakeIWBTGLevel(LevelIndividual goldenLevel)
    {
        ...
        float spatialNoiseScale = 0.25f;
        float trapDensityThreshold = 0.55f;
        ...
        for (int x = 1; x < map.mWidth - 1; x++)
        {
            for (int y = 1; y < map.mHeight - 1; y++)
            {
                if (map.GetTile(x, y) == TileType.Empty)
                {
                    ...
                    {
                        float noiseVal = Mathf.PerlinNoise(x * spatialNoiseScale, y * spatialNoiseScale);
                        ...
                        if (noiseVal > trapDensityThreshold)
                        {
                            ...
                            if (trapToSpawn != null)
                            {
                                GameObject instantiatedTrap = Instantiate(trapToSpawn, new Vector3(worldPos.x, worldPos.y, -5f), Quaternion.identity);
